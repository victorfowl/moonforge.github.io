...
// Rutas de i18n y datos (absolutas desde raíz)
const dicts = {
...
function renderPortfolioList(items) {
    ...
    items
        ...
        .forEach(p => {
            ...
            if (p.video) {
                const posterAttr = p.poster ? `poster="${assetUrl(p.poster)}"` : '';
                media.innerHTML = `
...
// ---------- Carrusel para páginas de proyecto ----------
function renderCarousel(root, media = [], legacyThumb, legacyVideo, legacyPoster) {
    ...
    media.forEach(m => {
        ...
        if (m.type === 'video' && m.src) {
            const posterAttr = m.poster ? `poster="${assetUrl(m.poster)}"` : '';
            slide.innerHTML = `
...
>>>>>>> parent of e75ec22 (Some style changes)
